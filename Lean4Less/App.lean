      ...
      if aEqb?.isSome || fEqg?.isSome then
        ...
        let elseCase := do
          ...
          if let .some UaEqVb := UaEqVb? then
            if let some fEqg := fEqg? then
              let hUV := {a := aVar, UaEqVb, extra := .none, alets := ← meth.getLets aVar}
              if let some aEqb := aEqb? then
        ...
        let extra ← if let .some (vaEqb, vbEqa, hAB) := d? then
          ...
          if UaEqVb?.isSome || dep then
            ...
            let hUV := {a := aVar, UaEqVb, extra := .some {b := bVar, vaEqb := {aEqb := vaEqb, bEqa := vbEqa, lets := ← meth.getLets vaEqb}, blets := ← meth.getLets bVar}, alets := ← meth.getLets aVar}
