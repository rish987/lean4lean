    ...
    try
      ...
      if not $ skipConsts.contains const then
        ...
        let rp modEnv := do
          let mut modEnv := modEnv
          if dbgOnly then
            let (env', _, _, _) ← replay addDeclFn {newConstants := map.erase const, overrides, opts := opts} modEnv.toKernelEnv (printProgress := printProgress) (op := op)
            modEnv := updateBaseAfterKernelAdd modEnv env'
          else
            if deps then
              let (env, _, _, _) ← replay addDeclFn {newConstants := map, overrides, opts} modEnv.toKernelEnv (printProgress := printProgress) (op := op)
              modEnv := updateBaseAfterKernelAdd modEnv env
            else
              for (_, ci) in map.erase const |>.toList do
                modEnv := updateBaseAfterKernelAdd modEnv (modEnv.toKernelEnv.add ci)
              let (env, _, _, _) ← replay addDeclFn {newConstants := Std.HashMap.insert default const (map.get! const), overrides, opts} modEnv.toKernelEnv (printProgress := printProgress) (op := op)
              modEnv := updateBaseAfterKernelAdd modEnv env
          pure modEnv
        ...
        if dbgOnly then
          ...
          let (modEnv', _, _, _) ← replay addDeclFn {newConstants := Std.HashMap.insert default const (map.get! const), overrides, opts} modEnv.toKernelEnv (printProgress := printProgress) (op := op)
