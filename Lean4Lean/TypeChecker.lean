...
algorithmic definitional equality judgement.
-/
...
might save some work by directly checking if these the major premises/struct
  ...
  | .app .. => -- beta-reduce at the head as much as possible, apply any remaining `rargs` to the resulting expression, and re-run `whnfCore`
    ...
    if let .lam _ _ body _ := f then
      let rec loop m (f : Expr) : RecM Expr :=
        -- FIXME only execute this when in one of the `else`s below (though the cost of `whnfCore` is minimal when `r` is a lambda)
        let cont2 := do
...
def reduceNat (e : Expr) : RecM (Option Expr) := do
  ...
  -- FIXME should return some if e is already a nat literal?
  return none
...
- one of them can be converted to a natural number literal.
...
def isDefEqCore' (t s : Expr) : RecM Bool := do
  ...
  -- FIXME shouldn't we have a similar optimization for the case t.isConstOf ``true?
